...
import TableHasilMagang, { type HasilMagang } from "./Components/TableHasilMagang";
import { Button } from "@/components/ui/button";
import { Download, Calendar, X } from "lucide-react";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";

const HasilMagangPage = () => {
  ...
  const fetchData = (start?: string, end?: string) => {
    ...
    axios.get("http://127.0.0.1:8000/api/admin/hasil-magang", {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Laporan Hasil Magang Polda Jateng", 14, y);
    y += 8;
    ...
    autoTable(doc, {
      startY: y,
      head: [["Nama", "Universitas", "Divisi", "Nilai", "Predikat"]],
      body: data.map((item) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <TableHasilMagang data={data} />
      )}
    </div>
  );
};

export default HasilMagangPage;